...
| A full admin-notify channel drops the re-approval announcement after the edit commits | **Accepted** — identical to the create path's guarantee, logged at `Error`, and `GET /api/admin/pending-issues` remains authoritative so the issue is still reviewable. A durable outbox is the real fix and should cover create too; out of scope here. |
